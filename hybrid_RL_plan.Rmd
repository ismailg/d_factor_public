...
trustee_decision_hybrid_k <- function(params, data, plan_depth, use_priors = FALSE) {
  ...
  # sensitivity_game2, alpha_Q_game1, alpha_Q_game2, w.
  if (length(params) != 9) {
    stop("Requires 9 parameters: envy, guilt, temp1, temp2, sens1, sens2, alpha1, alpha2, w")
  }
  ...
  Q_MF <- matrix(0, nrow = 3, ncol = 6)  # model-free Q
...
fit_participant_hybrid_k <- function(participant_data,
  ...
  for (k_test in 1:max_k) {
    ...
    if (!is.null(best_fit_k)) {
      best_params_k <- setNames(as.numeric(best_fit_k[1, 1:9]), names(lower))
...
test_results_hyb_new <- fit_participant_hybrid_k(test_data1, max_k = 2, hierarchical = TRUE, n_multistart = 20)
cat("Estimated envy:", test_results_hyb_new$parameters[["envy"]], "\n")
```
...
fit_all_participants_hyb_k <- function(data,
  ...
  results_df <- do.call(rbind, lapply(seq_along(participants), function(i) {
    ...
    data.frame(
      ...
      w = if ("w" %in% names(fit$parameters)) fit$parameters[["w"]] else NA,
      nll = fit$nll,
      aic = fit$aic,
      bic = fit$bic,
      best_k = fit$k,
...
test_results_hyb_new <- fit_all_participants_hyb_k(test_data2, max_k = 2, hierarchical = TRUE, n_multistart = 5)
print(test_results_hyb_new)
```

```{r}
# Run the hierarchical fitting
results_hybrid_k <- fit_all_participants_hyb_k(final_data, max_k=3, hierarchical = TRUE, n_multistart=5)


# Print and summarize results
print(results_hybrid_k)
summary(attr(results_hybrid_k, "summary"))
write.csv(results_hybrid_k, "results_hybrid_k.csv", row.names = FALSE)
```
...
analyze_results <- function(results) {
    ...
    param_summary <- results %>%
        summarise(across(c(best_k, envy, guilt, alpha_Q_game1, alpha_Q_game1, temp_game1, temp_game2, sensitivity_game1, sensitivity_game2,w), 
...
analyze_results(results_hybrid_k)
```
...
ggpairs(
  results_hybrid_k,
  columns = c("w", "envy", "guilt", "temp_game1", "alpha_Q_game1","temp_game2", "alpha_Q_game2"),
...
simulate_game <- function(game_data, params, plan_depth, Q_MF_init = NULL, Q_MB_init = NULL) {
  ...
  for (t in seq_len(n_trials)) {
    ...
    # --- Prediction Step: Compute prediction BEFORE any updates ---
    ...
    # --- Learning Step: Now update Q values using the observed outcome ---
    payoffs <- calculate_payoffs(investment, return_amt)
    reward <- calculate_fs_utility(payoffs$trustee, payoffs$investor, 
                                   params["envy"], params["guilt"])
    
    # Model-free update (if not the last trial)
    if (t < n_trials) {
    ...
    # Model-based update using dynamic programming values
...
simulate_test_data <- function(test_data, params, plan_depth, training_Q_list) {
  ...
  for (g in games) {
    ...
    sim_result <- simulate_game(game_data, params, plan_depth,
                                Q_MF_init = Q_init$Q_MF, Q_MB_init = Q_init$Q_MB)
...
fit_and_test_participant <- function(participant_data, max_k = 3, hierarchical = TRUE, n_multistart = 20) {

  training_data <- participant_data %>% filter(roundNum <= 20)
  test_data     <- participant_data %>% filter(roundNum > 20)
  
  fit_results <- fit_participant_hybrid_k(training_data, max_k = max_k, hierarchical = hierarchical, n_multistart = n_multistart)
...
fit_and_test_participant(test_data_rand, max_k = 2, hierarchical = TRUE, n_multistart = 2)
```
...
fit_and_test_all_participants <- function(final_data, max_k = 3, hierarchical = TRUE, n_multistart = 20, n_cores = 4) {
  ...
  cl <- makeCluster(n_cores)
  clusterExport(cl, varlist = c("fit_participant_hybrid_k", "trustee_decision_hybrid_k",
                                ...
                                "transition_probs_state_action", "simulate_game", "get_training_Q",
                                "simulate_test_data", "fit_and_test_participant"),
                envir = environment())
  clusterEvalQ(cl, { library(tidyverse); library(optimx); library(lhs) })
  
  results_list <- parLapplyLB(cl, participants, function(p_data) {
    tryCatch({
      fit_and_test_participant(p_data, max_k = max_k, hierarchical = hierarchical, n_multistart = n_multistart)
    }, error = function(e) {
...
results_hybPlan_test <- fit_and_test_all_participants(final_data, max_k = 3, hierarchical = TRUE, n_multistart = 5, n_cores = 8)

# Display the results:
print(results_hybPlan_test)

write.csv(results_hybPlan_test, "results_hybPlan_test.csv", row.names = FALSE)


# The results_df contains, for each participant:
#  - playerId
#  - test_nll: The out-of-sample negative log likelihood from the test rounds.
#  - bin_accuracy: The proportion of test rounds in which the model's predicted bin (argmax of softmax) matched the observed bin.
#  - best_k: The planning depth chosen during fitting.
```
...
analysis_data_long <- results_hybrid_k %>%
...
```{r}
# Model temp as a function of (opponent.f, d_level, etc.), random intercept by participant
mod_sensitivity <- afex::mixed(
  sensitivity ~ opponent.f*d_level + (1 | participant), 
  data = analysis_data_long
)
...
```{r}
# global parameters 
analysis_data_global <- results_hybrid_k %>%
...
ggpairs(
  vars_of_interest,
  columns = c("w", "envy", "guilt", "mean_payoff", "cal_scaled","dec_scaled", "sad_scaled", "vin_scaled" ),
  title = "Pairwise Parameter Relationships"
)
```
...
```{r}
model_payoff <- lm(
  mean_payoff ~  w + envy + guilt + best_k + cal_scaled + dec_scaled + sad_scaled + vin_scaled,
  data = analysis_data_global
)
...
analysis_data_global$best_k <- ifelse(analysis_data_global$best_k == 1, 2, analysis_data_global$best_k)
