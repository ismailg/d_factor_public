...
fit_direct_model_robust <- function(participant_data,
  ...
  objective_transformed <- function(unconstrained_params) {
    ...
    # Add very small L2 regularization
    reg_term <- 1e-6 * sum(constrained_params^2)
    return(direct_learning_model(constrained_params, participant_data, param_fixed) + reg_term)
  }
  ...
  if ("alpha" %in% free_names && "temp" %in% free_names) {
    # Generate a wider range of starting points that cover the trade-off relationship
    alpha_temp_pairs <- list(
      c(0.1, 10.0),  # Low alpha, high temp
      c(0.5, 5.0),   # Medium alpha, medium temp
      c(0.9, 1.0)    # High alpha, low temp
    )
    
    for (pair in alpha_temp_pairs) {
      ...
      init[idx_alpha] <- pair[1]
      init[idx_temp] <- pair[2]
      
      # Add guilt values if it's a free parameter
      if ("guilt" %in% free_names) {
        idx_guilt <- which(free_names == "guilt")
        init[idx_guilt] <- runif(1, 0, 0.8)  # Focus on lower values based on grid search
      }
  ...
  for (i in seq_along(init_list)) {
    ...
    # METHOD 2: L-BFGS-B to refine result
      ...
      # METHOD 3: Try nlminb optimizer for better boundary handling
      fit_nlminb <- tryCatch({
        nlminb(start = fit_global$par,
               objective = objective_transformed,
               control = list(eval.max = 1000, iter.max = 1000))
      }, error = function(e) {
        if (verbose) cat("nlminb optimization failed:", e$message, "\n")
        NULL
      })
      
      if (!is.null(fit_nlminb)) {
        val <- fit_nlminb$objective
        if (val < best_nll) {
          best_nll <- val
          best_par <- transform_params(fit_nlminb$par)
          
          if (verbose) {
            cat("New best value from nlminb:", val, "with params:", best_par, "\n")
          }
        }
        
        # Store fit for diagnostics
        all_fits[[length(all_fits) + 1]] <- list(
          method = "nlminb after SANN",
          value = val,
          params = transform_params(fit_nlminb$par),
          converged = fit_nlminb$convergence == 0
        )
      }
    }
    
    # METHOD 4: Try Nelder-Mead directly
    fit_nm <- tryCatch({
      optim(par = init,
            fn = objective_transformed,
            method = "Nelder-Mead",
...
#  Fitting with all free parameters 
...
fit_result_MFRL_all_37 <- fit_all_participants_parallel(
  final_data,
  param_fixed=list(),
  n_multistart=10,
  n_cores=9  # Adjust based on your computer
)
# 7. Save results
write.csv(fit_result_MFRL_all_37, "fit_result_MFRL_all_37.csv", row.names=FALSE)
# 
# # 5. Or perform out-of-sample testing
oos_results_free <- fit_and_test_all_participants(
  final_data,
  param_fixed=list(),
  n_multistart=10,
  n_cores=9,
...
cat("\nOut-of-Sample Testing Results:\n")
cat("-----------------------------\n")
cat("Average bin prediction accuracy:", mean(oos_results_free$bin_accuracy, na.rm=TRUE), "\n")
cat("Convergence rate:", mean(!is.na(oos_results_free$training_fit_nll)), "\n")

# Print parameter summaries
for (param in c("alpha", "temp", "envy", "guilt")) {
    cat(param, "mean:", mean(oos_results_free[[param]], na.rm=TRUE), 
        "SD:", sd(oos_results_free[[param]], na.rm=TRUE), "\n")
  }
# 
...
# Fixing envy to 0 and temperature to 1 

```{r}
final_data <- read.csv("data/final_data.csv")
# 4. Fit the model with fixed parameters (parallel)
fit_result_MFRL_t1e0 <- fit_all_participants_parallel(
  final_data,
  param_fixed=list(temp=1, envy=0),
  n_multistart=10,
  verbose=TRUE
)

# Save the results
write.csv(fit_result_MFRL_t1e0 , "fit_result_MFRL_t1e0", row.names = FALSE)

# View summary of fitted parameters
summary(fit_result_MFRL_t1e0[, c("alpha", "temp", "envy", "guilt")])


################ OOS testing
# Or perform out-of-sample testing

fixed_params_t1e0 <- list(temp=1, envy=0)
oos_results_t1e0 <- fit_and_test_all_participants(
  final_data,
  param_fixed=fixed_params_t1e0,
  n_multistart=10,
  n_cores=9,
  train_cutoff=20  # Use first 20 rounds for training
)
...
cat("\nOut-of-Sample Testing Results:\n")
cat("-----------------------------\n")
cat("Average bin prediction accuracy:", mean(oos_results_t1e0$bin_accuracy, na.rm=TRUE), "\n")
cat("Convergence rate:", mean(!is.na(oos_results_t1e0$training_fit_nll)), "\n")

# Print parameter summaries
for (param in c("alpha", "temp", "envy", "guilt")) {
  if (param %in% names(fixed_params_t1e0)) {
    cat(param, "was fixed at", fixed_params_t1e0[[param]], "\n")
  } else {
    cat(param, "mean:", mean(oos_results_t1e0[[param]], na.rm=TRUE), 
        "SD:", sd(oos_results_t1e0[[param]], na.rm=TRUE), "\n")
  }
}
...
# Fixing envy to 1 and temp to 1.5
```{r}
# 3. Based on recovery results, determine which parameters to fix
# For example, if envy and guilt are poorly recovered, we might fix them:
fixed_params_t1.5_e1 <- list(temp=1.5, envy=1)

# 4. Fit the model with fixed parameters (parallel)
fit_result_MFRL_t15e1 <- fit_all_participants_parallel(
  final_data,
  param_fixed=fixed_params_t1.5_e1,
  n_multistart=10,
  verbose=TRUE
)

# Save the results
write.csv(fit_result_MFRL_t15e1 , "fit_result_MFRL_t15e1.csv", row.names = FALSE)

# View summary of fitted parameters
summary(fit_result_MFRL_t15e1[, c("alpha", "temp", "envy", "guilt")])

```

```{r}
# 5. Or perform out-of-sample testing
oos_results_t15_e1 <- fit_and_test_all_participants(
  final_data,
  param_fixed=fixed_params_t1.5_e1,
  n_multistart=10,
  n_cores=9,
...
cat("\nOut-of-Sample Testing Results:\n")
cat("-----------------------------\n")
cat("Average bin prediction accuracy:", mean(oos_results_t15_e1$bin_accuracy, na.rm=TRUE), "\n")
cat("Convergence rate:", mean(!is.na(oos_results_t15_e1$training_fit_nll)), "\n")

# Print parameter summaries
for (param in c("alpha", "temp", "envy", "guilt")) {
  if (param %in% names(fixed_params_t1.5_e1)) {
    cat(param, "was fixed at", fixed_params_t1.5_e1[[param]], "\n")
  } else {
    cat(param, "mean:", mean(oos_results_t15_e1[[param]], na.rm=TRUE), 
        "SD:", sd(oos_results_t15_e1[[param]], na.rm=TRUE), "\n")
  }
}
```
...
results_features_MFRLt15e1 <- fit_result_MFRL_t15e1 %>%
  left_join(
    payoff_data_mean, 
    by = c( "playerId")
  )
...
mod_alpha_mfrl <- lm( alpha ~ cal_scaled + dec_scaled + sad_scaled + vin_scaled + mean_payoff, data = results_features_MFRLt15e1)
summary(mod_alpha_mfrl)


# Model guilt 
mod_guilt_mfrl <- lm( guilt ~ cal_scaled + dec_scaled + sad_scaled + vin_scaled + mean_payoff, data = results_features_MFRLt15e1)
summary(mod_guilt_mfrl)

```

```{r}
results_features_MFRL_t1e0 <- fit_result_MFRL_t1e0 %>%
  left_join(
    payoff_data_mean, 
    by = c( "playerId")
  )
...
# Model alpha as a function of ( d_factor subscales and payoff, etc.)
mod_alpha_mfrl2 <- lm( alpha ~ cal_scaled + dec_scaled + sad_scaled + vin_scaled + mean_payoff, data = results_features_MFRL_t1e0)
summary(mod_alpha_mfrl2)


# Model guilt 
mod_guilt_mfrl2 <- lm( guilt ~ cal_scaled + dec_scaled + sad_scaled + vin_scaled + mean_payoff, data = results_features_MFRL_t1e0)
summary(mod_guilt_mfrl2)
```
