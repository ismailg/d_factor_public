...
Trust and cooperation are fundamental to human social interaction and economic exchange [@OstromWalker2003; @Yamagishi2011].
The trust game, introduced by @Berg1995, provides a powerful and widely adopted tool for investigating the dynamics of trust and trustworthiness in controlled settings [@Camerer2003; @JohnsonMislin2011].

While situational factors influence behavior, stable individual differences in personality traits are also significant predictors [@EvansRevelle2008; @MullerSchwieren2020].
Among the most potent predictors of antisocial or self-interested behavior are the "dark" personality traits.
Although often studied via the Dark Triad framework (Machiavellianism, Narcissism, Psychopathy; @PaulhusWilliams2002), the considerable overlap between these traits [@Zettler2021] spurred the development of the Dark Factor of Personality (D-factor) [@Moshagen2018].
Proposed as the common core underlying various malevolent traits, D is defined as "the general tendency to maximize one's utility at the expense of others, accompanied by beliefs that serve as justifications" [@Moshagen2018].
This unifying construct represents the shared variance among dark traits such as Machiavellianism and Psychopathy—traits previously linked to reduced trustworthiness [@Ibanez2016; @Gunnthorsdottir2002]—offering a parsimonious framework for understanding antisocial tendencies [@MoshagenHilbigZettler2025; @HilbigZettlerMoshagenThielmann2021; @Zettler2021].

Research consistently links dark traits and D to reduced cooperation and honesty [@Zhao2015; @Thielmann2019], particularly in one-shot economic games like the dictator game [@Hilbig2021; @HilbigThielmann2025].
However, these single-interaction studies cannot capture the dynamics crucial to real-world exchanges, such as reputation formation [@Bohnet2004] and strategic adaptation over time.
Understanding how D influences behavior in repeated exchanges is particularly important, especially given that the strategic manipulation characteristic of high-D individuals [@Jones2009] may only fully manifest across multiple interactions.
Yet, the specific impact of D in repeated trust games remains largely unexplored.
Prior work using repeated games often involved different paradigms (e.g., sequential partners; @Gong2019) or focused on clinical samples and related constructs [@Rosenberger2019], leaving a gap in understanding how D operates strategically in sustained interactions within the general population.
...
HMMs [@Rabiner1989] are computational models ideal for capturing behavioral sequences driven by underlying states.
...
We expect high-D and low-D individuals to show diverging patterns depending on opponent characteristics. D is strongly associated with distrust-related beliefs—including cynicism and expectations that others will act selfishly—which serve to justify exploitative behavior [@HilbigMoshagenThielmannZettler2022]. High-D individuals may therefore test whether exploitation is viable and adjust based on opponent feedback: exploiting predictable opponents who fail to punish non-reciprocation, while showing restraint when opponents respond adaptively and exploitation becomes costly. Low-D individuals, lacking these justificatory beliefs, should behave more consistently regardless of opponent type. This predicts an interaction between D-level and opponent responsiveness.
...
# Helper function for formatting p-values (APA style)
format_p <- function(p) {
  if (is.na(p)) {
    return("NA")
  } else if (p < 0.001) {
...
Based on the scale developers' validation sample ($N = 6{,}838$), the D16 demonstrated strong internal consistency (Cronbach’s $\alpha = .91$) and excellent test--retest reliability ($r = .90$), with confirmatory factor analyses supporting its single-factor structure [@MoshagenZettlerHilbig2020Assessment].
...
Prior to data collection, we determined our target sample size of 180 participants through a power analysis using Monte Carlo simulations with the *simr* package in R.
...
Parameters for the simulation were based on previous studies, with an expected effect size of $-0.1$ (correlation between d-score and returns, see @Thielmann2020personality), alpha level of $0.05$, and desired power of $0.90$.
...
The repeated-measures design (48 observations per participant across two games of 24 rounds each) yields approximately 8,800 total observations; this structure substantially increases power for detecting within-subject effects and cross-level interactions because it reduces residual variance through the estimation of participant-specific random effects.
...
The experimental interface was designed and implemented online using Empirica v1 [@Almaatouq2021], with an estimated completion time of 30 minutes per participant.
The study received approval from the University of Heidelberg ethics commission with ID: S-708/2023 and the experiment was performed in accordance with the ethics board guidelines and regulations.
...
The trust game is a two-player sequential game that captures the tension between self-interest and mutual benefit [@Berg1995]. An investor transfers part of their endowment to a trustee; this amount is multiplied (typically tripled), and the trustee decides how much to return. This creates a social dilemma: while keeping everything maximizes immediate payoff, returning a fair share can sustain cooperation and yield higher long-term gains for both parties.

Based on @Berg1995, participants always acted as the trustee.
...
Participants were paid a fixed fee of £4 plus a bonus payment dependent on their performance that averaged £0.50.
...
Computerised investors were implemented as Hidden Markov Models (HMMs) trained on an independent dataset of 381 10-round RTGs (see Supplement for details).
...
To clarify our analytic strategy: our *confirmatory* hypothesis, as specified in the power analysis, was the three-way interaction between D-factor, Opponent type, and Investment level—testing whether high-D individuals would show differential exploitation depending on opponent characteristics and investment magnitude.
...
Higher-order interactions involving Order and Round (i.e., four-way and five-way interactions) are treated as *exploratory* and interpreted with appropriate caution.
Random effects included participant-wise intercepts and slopes for Opponent type and Investment.
...
The model was estimated using the `afex` package [@Singmann2022] in R with Kenward-Roger approximation for degrees of freedom.
...
For significant interactions, we conducted planned contrasts using the `emmeans` package with Sidak corrections to control familywise error rates.
Model complexity was constrained by reliable estimation considerations, resulting in an optimal random effects structure [@Matuschek2017].
...
We also evaluated a growth model including quadratic trends; although the likelihood ratio test indicated the quadratic term significantly improved fit, D-factor did not interact with the quadratic term, and BIC favored the simpler specification; we therefore retained the linear model (see Supplementary Materials).
...
Data and relevant code for this research work are stored in GitHub: https://github.com/ismailg/d_factor_public and have been archived within the Zenodo repository: https://doi.org/10.5281/zenodo.18846593.
...
High-D participants received lower investments (`r papaja::apa_print(t_test_investment)$statistic`) and consistently returned less money to investors (`r papaja::apa_print(t_test_return)$statistic`).
The difference in return percentage was statistically significant (`r papaja::apa_print(t_test_return_pct)$statistic`), with High-D participants returning approximately 2-4 percentage points less of the tripled investment.
...
The final round (round 25) was excluded due to endgame effects—participants were informed it was the last interaction, which could distort typical behavior—and analysed separately.
...
We found a significant main effect of investment amount (`r papaja::apa_print(mod_return_pct)$full_result$inv_scaled`), with participants returning higher percentages when they received larger investments, demonstrating positive reciprocity.
We also found a significant main effect of round number (`r papaja::apa_print(mod_return_pct)$full_result$roundNum`), showing that return percentages generally decreased over time as the game progressed.
...
We found a significant interaction between D-factor and round number (`r papaja::apa_print(mod_return_pct)$full_result$d_level_roundNum`).
Participants with High-D scores demonstrated a significant negative slope in their return proportions as the game progressed, indicating a systematic decrease in reciprocity over time (slope = `r sprintf("%.4f", d_level_trends_test$roundNum.trend[1])`, 95% CI [`r sprintf("%.4f", d_level_trends_test$asymp.LCL[1])`, `r sprintf("%.4f", d_level_trends_test$asymp.UCL[1])`]).
...
The difference between these slopes was statistically significant (`r sprintf("z = %.2f, p = %.3f", summary(d_level_trends_contrast)$z.ratio, summary(d_level_trends_contrast)$p.value)`).
...
Exploratory analysis of the significant four-way interaction (`r papaja::apa_print(mod_return_pct)$full_result$opponent_f_inv_scaled_d_level_roundNum`), visualised in Figure \@ref(fig:oppExploit), revealed that only High-D participants facing the human-like opponent showed investment-dependent changes in behavior across rounds (`r format_p(relevant_contrast$p.value)`).
For these participants, returns decreased significantly across rounds with high investments (slope = `r sprintf("%.5f", high_d_human_high_inv$roundNum.trend)`, 95% CI [`r sprintf("%.5f", high_d_human_high_inv$asymp.LCL)`, `r sprintf("%.5f", high_d_human_high_inv$asymp.UCL)`]), but remained stable with low investments (slope = `r sprintf("%.5f", high_d_human_low_inv$roundNum.trend)`, 95% CI [`r sprintf("%.5f", high_d_human_low_inv$asymp.LCL)`, `r sprintf("%.5f", high_d_human_low_inv$asymp.UCL)`]), a significant difference in slopes (`r format_p(relevant_contrast$p.value)`).
...
```{r orderEffectPlot, include=T, fig.pos="H", fig.cap="Effect of game order on changes in return rate across rounds at different investment levels. Lines represent the estimated slope coefficients (change in return rate per round) with shaded areas showing 95 percent confidence intervals. Negative values indicate decreasing reciprocity over time. High-D participants who played against the Human-like opponent first showed strategic exploitation primarily at high investment levels, while those who faced the Responsive opponent first showed different strategic patterns. Low D-factor participants maintained more consistent return rates regardless of order condition.", fig.align="center",  out.width="\\textwidth", dev="pdf", fig.width=10, fig.height=6}
...
ggplot(trend_data, aes(x = inv_scaled, y = roundNum.trend,
  ...
  scale_color_manual(
    ...
    labels = c("Human-like First" = "Human-like First", "Responsive First" = "Responsive First")
  ) +
  scale_fill_manual(
    values = c("#7570b3", "#e7298a"),
    labels = c("Human-like First" = "Human-like First", "Responsive First" = "Responsive First")
...
### Four-Way Interaction: Investment, Order, D-factor, and Round Number (Exploratory)

The significant four-way interaction involving investment amount, order of opponent presentation (responsive first or stable/human-like first), D-factor, and round number (`r papaja::apa_print(mod_return_pct)$full_result$inv_scaled_d_level_volatile_first_roundNum`) reveals a complex interplay of factors influencing return behavior; as noted in our analytic strategy, this higher-order interaction is exploratory and should be interpreted with caution.
As seen in Figure \@ref(fig:orderEffectPlot), the key finding is that the *order* in which participants faced the opponents, combined with their D-factor level, influenced how their returns changed over time *depending on the investment level*.
...
High-D participants who faced the *stable* (human-like) *opponent* *first* showed a strategic pattern: they significantly decreased their returns over rounds for *high* (slope = `r sprintf("%.5f", high_D_stable_high$slope)`, `r format_p(high_D_stable_high$p_value)`) and *medium* investments (slope = `r sprintf("%.5f", high_D_stable_med$slope)`, `r format_p(high_D_stable_med$p_value)`), but not for low investments (slope = `r sprintf("%.5f", high_D_stable_low$slope)`, `r format_p(high_D_stable_low$p_value)`).
...
These ratings also serve as a manipulation check: the significant main effect of opponent type on cooperativeness and trustworthiness ratings confirms that participants perceived the intended behavioral differences between the two HMM types.
...
Our results demonstrated significant behavioral differences between individuals with High-D and Low-D scores.
...
This pattern aligns with the fundamental definition of the D-factor as "the general tendency to maximize one's utility at the expense of others, accompanied by beliefs that serve as justifications" [@Moshagen2018].
...
This temporal dimension of trustworthiness aligns with prior research suggesting that dark personality traits may manifest most strongly after establishing a baseline relationship [@Jones2009].
This finding extends previous work on dark traits in one-shot economic games [@Zhao2015; @Thielmann2019] by demonstrating how these tendencies unfold over repeated interactions.
...
This finding aligns with the conceptualization of Machiavellianism as involving strategic, long-term orientation to personal gain [@Jones2009] and supports previous research indicating that dark personality traits are associated with strategic rather than impulsive exploitation [@Gunnthorsdottir2002].
...
This interpretation is consistent with the temporal patterns we observed: exploitation emerged gradually over rounds rather than appearing immediately, suggesting High-D participants *learned* that reduced reciprocity was sustainable rather than entering with a pre-formed plan.
...
In reinforcement learning terms, High-D individuals may update their strategies more rapidly or hold weaker priors toward prosocial defaults, making cooperation less "sticky" once exploitation appears viable [@BearRand2016].
Research on specific trait expressions through which D may manifest—such as psychopathy and Machiavellianism—has documented altered prediction error processing and differential sensitivity to environmental feedback [@Gregory2015; @Atanassova2025], though D-factor represents the common core underlying these expressions rather than any single trait [@Moshagen2018].
...
We suggest this reflects a *strategy transfer* mechanism: high-D individuals appear to develop exploitation strategies during their first game that then carry over to shape behavior in their second game.
...
This pattern is consistent with research showing that people transfer learned opponent models between games [@GuennouniSpeekenbrink2022].
...
This negative perceptual bias is consistent with research suggesting that individuals with dark personality traits may have distorted social perceptions that justify exploitation [@Moshagen2018; @Zettler2021].
The interaction between D-factor, game order, and opponent type for trust ratings suggests complex differences in how High-D and Low-D individuals update their social perceptions based on experience.
...
The significant D-factor difference in this context aligns with previous research showing associations between the D-factor and selfish behavior in dictator games [@Hilbig2021].
...
This aligns with theoretical accounts suggesting that dark personality traits may represent frequency-dependent strategies that are most beneficial when rare in a population [@Mealey1995], as widespread exploitation would trigger universal defensive responses that limit its effectiveness.
...
Regarding the relationship between D and established personality dimensions, prior research has shown that D and low Honesty-Humility share substantial variance (~66%) but remain functionally distinct [@HilbigZettlerMoshagenThielmann2021]. D better captures distrust-related beliefs and lack of empathy—components theoretically outside Honesty-Humility's scope—while low Honesty-Humility better accounts for pretentiousness. Similarly, while low Agreeableness captures ill-temperedness, D more comprehensively captures utility-maximization accompanied by justificatory beliefs that directly maps onto strategic exploitation in economic games. When tested against 58 personality traits and related constructs, D emerged as the singular dispositional predictor of how individuals weigh their own versus others' utility [@HilbigThielmannZettlerMoshagen2023]. Our findings are thus best understood through the D framework, though they likely generalize to individuals low in Honesty-Humility given the substantial overlap.
Second, our results highlight the importance of considering the temporal dimension of trust and reciprocity.
The emerging differences between High-D and Low-D participants over repeated interactions suggest that single-round economic games may underestimate the influence of personality traits on economic behavior.
...
The finding that High-D individuals modulate their exploitation strategies based on opponent predictability may reflect sophisticated social intelligence rather than rigid antisocial tendencies, though as discussed above, differential learning dynamics offer an alternative explanation.
...
First, while we observed clear behavioral differences between High-D and Low-D individuals, our design measured only the common core (D) and cannot determine whether the observed patterns differ across specific dark trait expressions (e.g., whether individuals high in Machiavellianism show different strategic patterns than those high in narcissism).
